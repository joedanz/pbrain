...
deliver results that are both semantically relevant and keyword-precise. PBrain uses
...
4. **Multi-query expansion** — PBrain generates multiple search queries from a single
...
## Implementation in PBrain

PBrain implements hybrid search in `src/core/search/` using Postgres as the single
...
to ship hybrid search with RRF in PBrain v0.3 as the highest priority feature.

### 2025-04-01 — Shipped in v0.3

Hybrid search shipped as part of PBrain v0.3. Initial results show significant
