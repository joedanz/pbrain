...
describe('LocalStorage', () => {
  ...
  beforeAll(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'pbrain-storage-test-'));
...
describe('LocalStorage path traversal', () => {
  test('blocks upload path traversal via ../', async () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'pbrain-traversal-'));
    try {
...
describe('createStorage', () => {
  test('creates LocalStorage for backend: local', async () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'pbrain-factory-test-'));
    try {
