...
1. **Today's meetings.** For each meeting on the calendar:
   - Search pbrain for each participant by name
   - Read their pages from pbrain for compiled_truth context
   - Summarize: who they are, recent timeline, relationship to you
2. **Active deals.** List deal pages in pbrain filtered to active status:
...
4. **Recent changes.** Pages updated in the last 24 hours:
   - What changed and why (read timeline entries from pbrain)
5. **People in play.** List person pages in pbrain sorted by recency:
   - Updated in last 7 days
   - Have high activity (many recent timeline entries)
6. **Stale alerts.** From pbrain health check:
   - Pages flagged as stale that are relevant to today's meetings

## PBrain-Native Context Loading

Before generating any briefing, load context from pbrain systematically.

### Before a meeting

For every attendee on the calendar invite:
- `pbrain search "<attendee name>"` -- find their brain page
- `pbrain get <slug>` -- load compiled truth, recent timeline, relationship context
...
- `pbrain search "<sender name>"` -- load sender context
...
- `pbrain query "active deals status"` -- deal pipeline snapshot
- `pbrain query "meetings this week"` -- recent meeting pages with insights
- `pbrain query "pending commitments follow-ups"` -- open threads and action items
- `pbrain search --type person --sort updated --limit 10` -- people in play
...
- **Briefing without brain queries.** Never generate a briefing from memory alone; always query pbrain for current data.
