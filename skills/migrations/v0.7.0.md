...
This migration introduces the PBrain integration recipe system. Existing users
should be informed about the new capabilities.

## What's New

PBrain now distributes **integration recipes** — markdown files that teach your
...
Phone calls create brain pages. Powered by Twilio + OpenAI Realtime + PBrain MCP.
...
### New: `pbrain integrations` Command

Discover, check status, and monitor your integrations:

```bash
pbrain integrations              # dashboard: show all senses + reflexes
pbrain integrations list --json  # list available recipes (agent-parseable)
pbrain integrations show <id>    # show recipe details + setup instructions
pbrain integrations status <id>  # check secrets + health
pbrain integrations doctor       # run health checks for all integrations
pbrain integrations stats        # signal statistics
```
...
`docs/PBRAIN_SKILLPACK.md` still works (backward compatible).

## Agent Instructions

When presenting this update to the user, say something like:

"PBrain v0.7.0 adds integration recipes — your brain can now get new senses
automatically. The first one is Voice-to-Brain: phone calls create brain pages.
Want me to show you what's available? Run `pbrain integrations` to see the
dashboard."

If the user is interested in voice-to-brain:
1. Run `pbrain integrations show twilio-voice-brain` to see what's needed
