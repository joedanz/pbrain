...
```bash
ls ~/.pbrain/integrations/twilio-voice-brain/heartbeat.jsonl 2>/dev/null
```
...
- Is there a public URL configured in `~/.pbrain/config.json`?
...
with `pbrain integrations show twilio-voice-brain`." Skip to Step 6.
...
`pbrain integrations show twilio-voice-brain`."
...
echo '{"version":"0.8.0","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","voice_setup":"RESULT"}' >> ~/.pbrain/migrations/completed.jsonl
```

Tell the user: "v0.8.0 migration complete. Run `pbrain integrations doctor` to
