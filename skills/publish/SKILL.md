...
This is a **code + skill pair**: the deterministic code (`pbrain publish`) does
...
pbrain publish brain/companies/acme.md

# Password protected (auto-generate password)
pbrain publish brain/companies/acme.md --password

# Password protected (specific password)
pbrain publish brain/companies/acme.md --password "secret123"

# Custom title
pbrain publish brain/companies/acme.md --password --title "Acme -- Deal Analysis"

# Custom output path
pbrain publish brain/companies/acme.md --out /tmp/acme-share.html
```
...
```bash
pbrain publish brain/people/jane-doe.md --password --out ~/Desktop/jane-briefing.html
```
...
pbrain publish brain/companies/acme.md --password "secret" --out /tmp/acme.html

# Upload to Supabase Storage
pbrain files upload /tmp/acme.html --page shares/acme

# Get a signed URL (1-hour expiry)
pbrain files signed-url shares/acme/acme.html
```
...
```bash
pbrain publish brain/trips/japan-2026.md --out trip.html
...
```bash
pbrain publish brain/companies/acme.md --password "same-password" --out shares/acme.html
```
...
- **Assuming the user wants raw markdown.** The publish command produces beautiful HTML. Don't copy-paste markdown when `pbrain publish` exists.
...
- `pbrain publish` -- deterministic HTML generation (no LLM calls)
- `pbrain files upload` -- upload to cloud storage (optional)
- `pbrain files signed-url` -- generate access links (optional)
