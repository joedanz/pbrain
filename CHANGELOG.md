...
- **PBrain is now a Claude Code plugin — one command to install, no repo clone.** Install with `/plugin marketplace add joedanz/pbrain` then `/plugin install pbrain` and you get all 26 skills auto-discovered in Claude Code. No `bun install`, no `bun link`, no symlinks to manage. Your agent can start filing notes into your Obsidian vault, maintaining the wikilink graph, and enriching people/company pages immediately. Skills-only distribution — for full brain functionality (hybrid search, query, autopilot, cron), install the pbrain CLI separately. Claude Code's plugin system handles version pinning and upgrades automatically; a git pull on the marketplace updates you to the latest skills. `pbrain install-skills` remains for Cursor, Windsurf, and anyone running the full stack from a clone.
