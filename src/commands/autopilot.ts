/**
 * pbrain autopilot — Self-maintaining brain daemon.
 *
 * Runs: sync → extract → embed → backlinks fix in a continuous loop.
 * Health-based adaptive scheduling. Best-effort per step.
 *
 * Usage:
 *   pbrain autopilot [--repo <path>] [--interval N] [--json]
 *   pbrain autopilot --install [--repo <path>]
 *   pbrain autopilot --uninstall
 *   pbrain autopilot --status [--json]
 */
...
function logError(phase: string, e: unknown) {
  ...
  console.error(line);
  try {
    const logDir = join(process.env.HOME || '', '.pbrain');
...
export async function runAutopilot(engine: BrainEngine, args: string[]) {
  ...
  if (!repoPath) {
    console.error('No repo path. Use --repo or run pbrain sync --repo first.');
    process.exit(1);
  }

  // Lock file to prevent concurrent instances (#14)
  const lockPath = join(process.env.HOME || '', '.pbrain', 'autopilot.lock');
  try {
    mkdirSync(join(process.env.HOME || '', '.pbrain'), { recursive: true });
...
function plistPath(): string {
  return join(process.env.HOME || '', 'Library', 'LaunchAgents', 'com.pbrain.autopilot.plist');
}
...
async function installDaemon(engine: BrainEngine, args: string[]) {
  ...
  const pbrainDir = join(home, '.pbrain');
  mkdirSync(pbrainDir, { recursive: true });
  ...
  const wrapperPath = join(pbrainDir, 'autopilot-run.sh');
  const pbrainPath = process.execPath;
  // Shell-escape values to prevent command injection (#1)
  const safeRepoPath = repoPath.replace(/'/g, "'\\''");
  const safePbrainPath = pbrainPath.replace(/'/g, "'\\''");
  const wrapper = `#!/bin/bash
# Auto-generated by pbrain autopilot --install
# Sources shell profile for API keys, then runs autopilot
source ~/.zshrc 2>/dev/null || source ~/.bashrc 2>/dev/null || true
exec '${safePbrainPath}' autopilot --repo '${safeRepoPath}'
`;
...
<plist version="1.0">
<dict>
  <key>Label</key><string>com.pbrain.autopilot</string>
  <key>ProgramArguments</key><array>
    <string>${escapeXml(wrapperPath)}</string>
  </array>
  <key>RunAtLoad</key><true/>
  <key>KeepAlive</key><true/>
  <key>StandardOutPath</key><string>${escapeXml(home)}/.pbrain/autopilot.log</string>
  <key>StandardErrorPath</key><string>${escapeXml(home)}/.pbrain/autopilot.err</string>
</dict>
</plist>`;
    ...
    try {
      ...
      console.log(`Installed launchd service: com.pbrain.autopilot`);
      console.log(`  Repo: ${repoPath}`);
      console.log(`  Log: ~/.pbrain/autopilot.log`);
      console.log(`  Uninstall: pbrain autopilot --uninstall`);
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : String(e);
    ...
    const cronLine = `*/5 * * * * '${safeWrapperPath}' >> '${home.replace(/'/g, "'\\''")}/.pbrain/autopilot.log' 2>&1`;
    try {
      const existing = execSync('crontab -l 2>/dev/null || true', { encoding: 'utf-8' });
      if (existing.includes('pbrain autopilot') || existing.includes('autopilot-run.sh')) {
        console.log('Crontab entry already exists. Remove with: pbrain autopilot --uninstall');
        return;
      }
      // Use a temp file instead of echo pipe to avoid shell escaping issues (#1)
      const tmpFile = join(pbrainDir, 'crontab.tmp');
      writeFileSync(tmpFile, existing.trimEnd() + '\n' + cronLine + '\n');
      execSync(`crontab '${tmpFile.replace(/'/g, "'\\''")}'`, { stdio: 'pipe' });
      try { require('fs').unlinkSync(tmpFile); } catch {}
      console.log('Installed crontab entry for pbrain autopilot (every 5 minutes)');
      console.log(`  Uninstall: pbrain autopilot --uninstall`);
    } catch (e: unknown) {
...
function uninstallDaemon() {
  const home = process.env.HOME || '';
  const wrapperPath = join(home, '.pbrain', 'autopilot-run.sh');
  ...
  if (process.platform === 'darwin') {
    try {
      ...
      console.log('Uninstalled launchd service: com.pbrain.autopilot');
    } catch (e: unknown) {
    ...
    try {
      ...
      const filtered = existing.split('\n').filter(l =>
        !l.includes('pbrain autopilot') && !l.includes('autopilot-run.sh')
      ).join('\n');
      const tmpFile = join(home, '.pbrain', 'crontab.tmp');
      ...
      console.log('Removed crontab entry for pbrain autopilot');
    } catch (e: unknown) {
      console.error(`Failed to uninstall: ${e instanceof Error ? e.message : e}`);
    }
  }
...
function showStatus(json: boolean) {
  const logFile = join(process.env.HOME || '', '.pbrain', 'autopilot.log');
  let lastLine = '';
  try {
  ...
  if (process.platform === 'darwin') {
    ...
    try {
      ...
      installed = crontab.includes('pbrain autopilot');
    } catch { /* no crontab */ }
  }
