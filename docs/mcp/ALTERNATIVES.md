...
PBrain's MCP server runs via `pbrain serve` (stdio transport). To make it
...
implemented. Currently, remote MCP requires a custom HTTP wrapper around `pbrain serve`.
