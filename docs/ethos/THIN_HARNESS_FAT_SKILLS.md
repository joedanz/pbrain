...
When building PBrain features, use this decision guide:
...
**PBrain examples:**
- `pbrain integrations list` = **Code** (reads files, checks env vars, deterministic)
- `pbrain integrations status` = **Code** (checks env vars + heartbeat, deterministic)
- `pbrain integrations doctor` = **Code** (runs health checks, deterministic)
- `pbrain integrations stats` = **Code** (aggregates JSONL, deterministic)
