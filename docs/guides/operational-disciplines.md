...
on every_inbound_message(message):
    ...
    for entity in entities:
        existing = pbrain search "{entity.name}"
        if existing:
            pbrain add_timeline_entry <entity_slug> \
                --entry "{what_was_said}" \
    ...
    for idea in originals:
        pbrain put originals/{slug} --content "{user's exact phrasing}"
...
on information_needed(topic):
    ...
    brain_result = pbrain search "{topic}"
    if brain_result:
        page = pbrain get <slug>
...
on brain_write_complete():
    ...
    # The page you just wrote won't appear in pbrain search or pbrain query
...
on nightly_schedule("02:00"):
    ...
    pages = pbrain list_pages
    for page in pages:
        mentions = extract_entity_mentions(page.content)
        existing_links = pbrain get_links <page.slug>
        for mention in mentions:
            if mention not in existing_links:
                pbrain add_link <page.slug> <mention_slug>  # fix broken graph
    ...
    for page in stale_pages(older_than="7d"):
        timeline = pbrain get_timeline <page.slug>
        if timeline.has_new_entries_since_last_consolidation:
            ...
            pbrain put <page.slug> --content updated_truth
...
5. **`pbrain doctor` catches silent failures.** Embedding pipelines can stall, sync can fail silently, database connections can drop. The daily heartbeat catches these before they compound into data loss.

## How to Verify

1. Send a message mentioning a person with a brain page. Confirm the agent detects the entity and adds a timeline entry to their page (`pbrain get_timeline <slug>`).
2. Ask the agent about someone in the brain. Confirm it runs `pbrain search` or `pbrain get` BEFORE reaching for external APIs (check the tool call order).
3. Write a new page with `pbrain put`, then immediately run `pbrain search` for it. Confirm it appears in results (verifies sync ran).
4. Run `pbrain doctor`. Confirm it returns a health report with database status, page count, and any flagged issues.
5. After a dream cycle runs, check a page that had unlinked entity mentions. Confirm new links were added (`pbrain get_links <slug>`).
