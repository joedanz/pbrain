...
update_brain_page(slug, new_info, source):
  ...
  pbrain add_timeline_entry {slug} {
    date: today,
...
3. **PBrain search weights compiled truth higher.** `pbrain query` returns compiled
...
4. **The --- separator matters.** PBrain uses the first standalone `---` after
...
2. **Search for the person.** `pbrain query "Sarah Chen"`. The compiled truth
...
*Part of the [PBrain Skillpack](../PBRAIN_SKILLPACK.md). See also: [Source Attribution](source-attribution.md), [Entity Detection](entity-detection.md)*
