...
lookup(name_or_topic):
  ...
  results = pbrain search "{name_or_topic}"
  if results.length > 0:
    page = pbrain get {results[0].slug}
    return page  // done, brain had it

  // STEP 2: Hybrid search (needs embeddings, finds semantic matches)
  results = pbrain query "what do we know about {name_or_topic}"
  if results.length > 0:
    page = pbrain get {results[0].slug}
    return page

  // STEP 3: Direct slug (if you know or can guess the slug)
  page = pbrain get "people/{slugify(name_or_topic)}"
  if page: return page
...
2. **Fuzzy slug matching.** `pbrain get` supports fuzzy matching. If the exact
...
*Part of the [PBrain Skillpack](../PBRAIN_SKILLPACK.md). See also: [Brain-Agent Loop](brain-agent-loop.md), [Search Modes](search-modes.md)*
